...
| GET | `/api/v1/pools/defaults` | viewer | What a pool that says nothing is: chiefly the CPU and memory one runner gets. Every pool has a size — a request that names none is saved with this one — so a form opens its sliders on what the fleet would actually apply rather than on a constant of its own. |
