...
| Its kept tool cache | A directory `cache/tools/<pool>` in the host's shared folder, `/var/lib/zoomies/shared` — with the repository appended for a repository-scoped cache — when the pool sets `cache.tools` | Never trimmed or deleted by Zoomies; it is a cache, so emptying it costs the next job its downloads and nothing else. Created writable by every account, under a shared folder only Zoomies' own account can enter. |
