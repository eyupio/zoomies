...
| `host.resources_unknown` | note | One or more hosts have reported no CPUs, no memory and no disk. They are placed by slot count alone, which is how every host was placed before agents learnt to measure themselves — so nothing is broken, but a pool's resource limits cannot be fitted against a machine nobody has measured, and every figure on the host's card is missing. It is a note rather than a warning for that reason. Upgrading the agent fixes it on the next heartbeat, with no re-join. |
