...
| `zoomies restore --from-remote <name> [<id>\|latest]` | The same restore, on a host that has the configuration file and the encryption key and nothing else: the copy is fetched from that remote into the backup directory, decrypted with the remote's passphrase — or `--passphrase` for one sealed with a passphrase the configuration no longer carries — verified, and then restored. Naming no backup lists what the remote holds, because an operator in front of an empty machine has no way to know the ids. |
