...
| ZF-404 | What the agent owns on a host, written down | extension; documentation and one default | `in_progress` | decision 25 | — | The code is non-invasive already (no package upgrade, firewall change, reboot or container, image or volume prune anywhere in the tree); the one exception is the daemon-wide builder-cache prune `agent.docker_build_cache_mb` (default 5120) performs every five minutes, which decision 25 keeps and names. Remaining: the 'what the agent owns' section on the security and private-hosts pages, linked from Add-a-host. **Record, 23 September 2026:** `in_progress`: open in [#436](https://github.com/eyupio/zoomies/pull/436), not merged — one "What the agent owns on a host" section in `docs/security.md`, linked from the private-hosts page and the Add-a-host command step, with backend and docs tests pinned to it. |
