...
| ZF-214 | Shared provider lifecycle, Proxmox first (214a–c) | extension/new boundary | `in_progress` | 214a: ZF-210a contract; 214b: ZF-207/208, ZF-210a, ZF-404 ownership/drain and recovery gates; 214c: qualified 214b | [PR #263](https://github.com/eyupio/zoomies/pull/263) | Owner selected Proxmox on 13 September 2026. 214a in progress: the contract, its fake and the conformance suite, the `providers`/`machines` schema and state machine, the pure machine-demand calculation, and the capacity signal extended to fire when no host can run a pool at all. 214b follows in the same branch. **214c not started, and live qualification is not claimed**: fixture success is not qualification, and the 20-cycle procedure in [ROADMAP.md](../ROADMAP.md) section 9 needs a disposable cluster this work has no access to. Optional GARM experiment untouched and non-blocking |
