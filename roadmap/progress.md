...
| ZF-214 | Shared provider lifecycle, Proxmox first (214a–c) | extension/new boundary | `in_progress` | 214a: ZF-210a contract; 214b: ZF-207/208, ZF-210a, ZF-404 ownership/drain and recovery gates; 214c: qualified 214b | [PR #263](https://github.com/eyupio/zoomies/pull/263) | Owner selected Proxmox on 13 September 2026. 214a complete: the contract, its fake and the conformance suite, the `providers`/`machines` schema and eleven-state machine, the durable reconciler on its own loop, the pure machine-demand calculation, and the capacity signal extended to fire when no host can run a pool at all. 214b complete: the Proxmox VE client, ownership marks, preflight, discovery, the provider itself, the REST and CLI surface, the Providers and Machines pages, and the runbook in [docs/proxmox.md](../docs/proxmox.md). **Live qualification is written but not run, and is not claimed.** The 20-cycle procedure in [ROADMAP.md](../ROADMAP.md) section 9 is implemented as an `e2e`-tagged harness behind `make test-e2e-proxmox`, whose budget, ledger, preflight, inventory check and evidence record are compiled and tested in ordinary CI; every row of [proxmox-qualification.md](validation/proxmox-qualification.md) still reads `not run`, because it needs a disposable cluster this work has no access to. Fixture success is not qualification. **214c not started**: it needs demonstrated demand and live access to a second platform. Optional GARM experiment untouched and non-blocking |
| ZF-215 | Capacity fallback and scheduled readiness | extension | `not_started` | ZF-208; qualified ZF-214b for provider fallback | Planning only | [September comparison](competitive-review-2026-09.md); acceptance in [ROADMAP.md](../ROADMAP.md) section 9 |
...
| ID | Package | Classification | Status | Depends on | Session | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| ZF-222 | A status view for the audience that cannot sign in | extension | `not_started` | owner decision 29; ZF-207's problem split; ZF-202 | Planning only | Planned only: no route, setting, page or test exists. Blocked on decision 29 — whether any fleet fact may be read without an account — which is the whole of the package; acceptance in [ROADMAP.md](../ROADMAP.md) section 9. |
