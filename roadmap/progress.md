...
| ZF-223 | A report per installation | new | `not_started` | ZF-209 | — | Planned only: ZF-303's evidence helper as a product feature — per-installation counts and exact percentiles from the stored timestamps and `fault_kind`, answering past retention from ZF-209's roll-up. Acceptance in [ROADMAP.md](../ROADMAP.md) section 8. |
...
| ZF-404 | What the agent owns on a host, written down | extension; documentation and one default | `not_started` | decision 25 | — | The code is non-invasive already (no package upgrade, firewall change, reboot or container, image or volume prune anywhere in the tree); the one exception is the daemon-wide builder-cache prune `agent.docker_build_cache_mb` (default 5120) performs every five minutes, which decision 25 keeps and names. Remaining: the 'what the agent owns' section on the security and private-hosts pages, linked from Add-a-host. |
