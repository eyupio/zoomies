...
| ID | Package | Classification | Status | Depends on | Session | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| ZF-005 | Corrections from reading the documentation against the fleet, and the marketplace repin | documentation; one pin | `not_started` | nothing | — | Three pages disagree with the record: `docs/index.md` says Windows is not supported above the paragraph saying the agent is built; the support matrix and `docs/index.md` say nothing has started a container while CI runs in Docker-backend runners on a Zoomies fleet; `docs/proxmox.md` says qualified where [validation/proxmox-qualification.md](validation/proxmox-qualification.md) says not run. `deploy/marketplace/release.env` pins `v1.0.0`. Acceptance in [ROADMAP.md](../ROADMAP.md) section 8. |
