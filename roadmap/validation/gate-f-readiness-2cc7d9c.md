...
| | |
| --- | --- |
| Commit | `2cc7d9c` on `claude/roadmap-implementation-1tui5x` |
| Go | 1.26.0 (CI: 1.26) |
| Node | 22.22.2 (CI: 22) |
| Machine | The session's container, Linux amd64, no Docker daemon. CI: `blacksmith-2vcpu-ubuntu-2404` |
| Packages under test | 23 Go packages, `-race`; 275 Playwright cases across `chromium`, `mobile` and `first-run`, of which **260 passed and 15 skipped** — the skips are the cases whose fixture needs something this container has not got, and by this directory's own rule they are *not run* rather than passed; 2 runtime drills plus a budget test |
