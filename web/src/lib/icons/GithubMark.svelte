<!--
  The GitHub mark. Lucide dropped brand icons, so this is the octocat
  silhouette drawn as a filled glyph rather than Lucide's stroked style --
  the shape is not ours to redraw, only to size and colour like the icons
  beside it.
-->
<script lang="ts">
  import type { LucideProps } from '@lucide/svelte';

  let { size = 24, ...rest }: LucideProps = $props();
</script>
